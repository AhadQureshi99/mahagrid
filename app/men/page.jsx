...
import { getMenProductsGroupedByCategory } from "../server/fetching/fetch";

const page = async () => {
  const groupedProducts = await getMenProductsGroupedByCategory();
          ...
          {Object.entries(groupedProducts).map(([category, products]) => (
            <div key={category}>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {products.map((item) => (
  <Link href={`/men/${item._id}`} key={item._id}>
